...
SOLD_L = r"Sold by|Venduto da|Venditore|Vendu par|Vendeur|Vendido por|Vendedor|Verkauf durch|Verkäufer"
...
def prefer_amazon(get, store, asin, raw):
    ...
    Returns Amazon's offer with the marketplace offer attached as "alt", or the original page."""
    ...
    try:
        ...
        return raw
    if (is_blocked(own) or own.get("status") or seller_kind(own) != "amazon"
            or not (own.get("price") or own.get("price2"))):
        return raw
                    ...
                    try:
                        ...
                        if (not is_blocked(raw) and raw.get("title")
                            ...
                            if seller_kind(raw) == "other":
                                ...
                                raw = prefer_amazon(get, store, asin, raw)
...
class StoreClient:
    """One marketplace session for a product check, using the configured transport."""
        ...
        if TRANSPORT == "browser":
            if self.browser is None:
                ...
                self.browser = BrowserClient(self.store, DOMAIN[self.store], request_cookies(self.jar, self.store),
                                             KEEP_COOKIES, DATA_DIR)
            raw, _ = self.browser.fetch(asin, parse, self.jar, query)
        else:
...
def check_store(job, store, model):
    ...
    try:
        ...
        if page_matches(raw, model):
            raw = prefer_amazon(lambda q: client.product(job["asin"], q), store, job["asin"], raw)
        ...
        if model:
            for hit in client.search(model)[:3]:
                ...
                if page_matches(raw2, model):
                    raw2 = prefer_amazon(lambda q: client.product(hit["asin"], q), store, hit["asin"], raw2)
